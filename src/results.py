...
def inference(args):            # test dataset transcription and results
    ...
    for path in paths:
        with h5py.File(path, 'r') as hf:
            if hf.attrs['split'].decode() == split:
                ...
                targets_processor = TargetProcessor(segment_seconds=len(audio)/sample_rate,frames_per_second=frames_per_second, begin_note=begin_note, classes_num=classes_num)
                (target_dict, note_events) = targets_processor.process(start_time=0,midi_events_time=midi_events_time,midi_events=midi_events)
...
def calculate_metrics(args, thresholds=None):           # Calculate evaluation scores
    workspace = args.workspace
    model_type = args.model_type
    dataset = args.dataset
    split = args.split
